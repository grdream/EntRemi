...
<body class="font-sans antialiased bg-surface-950 text-white overflow-x-hidden">
    ...
    {{-- Nav --}}
    <nav class="fixed top-0 inset-x-0 z-50 flex items-center justify-between px-6 lg:px-16 h-16 bg-surface-950/80 backdrop-blur-xl border-b border-white/5">
        ...
        <div class="flex items-center gap-3">
            ...
            <a href="{{ route('login') }}" class="px-4 py-2 text-sm font-medium text-surface-300 hover:text-white transition-colors">Sign In</a>
    ...
    <section class="relative min-h-screen flex items-center pt-16">
        ...
        <div class="relative z-10 max-w-6xl mx-auto px-6 lg:px-16 py-24 text-center">
            ...
            <h1 class="text-5xl sm:text-6xl lg:text-7xl font-extrabold leading-tight mb-6 animate-slide-up">
                Track Every Show.<br>
                <span class="bg-clip-text text-transparent bg-gradient-to-r from-brand-400 via-brand-300 to-accent-400">Never Miss an Episode.</span>
            </h1>
            <p class="text-lg sm:text-xl text-surface-400 max-w-2xl mx-auto mb-10 leading-relaxed animate-slide-up" style="animation-delay:.1s">
            ...
            <div class="flex flex-col sm:flex-row items-center justify-center gap-4 animate-slide-up" style="animation-delay:.2s">
                ...
                <a href="{{ route('login') }}" class="inline-flex items-center gap-2 px-8 py-3.5 rounded-xl font-semibold text-surface-300 border border-surface-700 hover:border-surface-500 hover:text-white transition-all duration-200 text-base">
                    Sign In
            ...
            <div class="flex flex-wrap justify-center gap-3 mt-12 animate-fade-in" style="animation-delay:.4s">
                ...
                <span class="px-3 py-1 rounded-full bg-surface-800/60 border border-surface-700/50 text-xs text-surface-400">{{ $tag }}</span>
                @endforeach
            </div>
        </div>
    </section>
    ...
    <section class="py-24 relative">
        <div class="max-w-6xl mx-auto px-6 lg:px-16">
            <div class="text-center mb-16">
                <h2 class="text-3xl sm:text-4xl font-bold mb-4">Everything you need to stay on track</h2>
                <p class="text-surface-400 max-w-xl mx-auto">From auto-fetching show details to firing reminders at exactly the right time — WatchList Reminder does it all.</p>
            </div>
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <div class="group p-6 rounded-2xl bg-surface-900/60 border border-surface-800/60 hover:border-brand-500/30 hover:bg-surface-900 transition-all duration-300 hover:-translate-y-1">
                    ...
                    <h3 class="text-base font-semibold text-white mb-2">{{ $feat['title'] }}</h3>
                    <p class="text-sm text-surface-400 leading-relaxed">{{ $feat['desc'] }}</p>
                </div>
                @endforeach
            </div>
        </div>
    ...
    <section class="py-24 relative">
        <div class="absolute inset-0 overflow-hidden">
            <div class="absolute inset-x-0 bottom-0 h-64 bg-gradient-to-t from-brand-900/20 to-transparent"></div>
        </div>
        <div class="relative max-w-3xl mx-auto px-6 text-center">
            <h2 class="text-4xl font-extrabold mb-4">Ready to never miss an episode?</h2>
            <p class="text-surface-400 mb-8">Join WatchList Reminder. It's completely free to start.</p>
    ...
    {{-- Footer --}}
    <footer class="border-t border-surface-800/50 py-8 text-center text-sm text-surface-500">
